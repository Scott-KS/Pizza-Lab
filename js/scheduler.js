...
import { PIZZA_RECIPES, calculateDough } from '../recipes.js';
import {
  FERMENT_METHODS,
  getAvailableFermentMethods,
  buildScheduleBackward,
...
datetimeInput.value = new Date(tomorrow.getTime() - tzOffset).toISOString().slice(0, 16);
...
function preValidateSchedule() {
  ...
  if (!recipe) return;
  const sizeKey =
    (sizeSelect && sizeSelect.value) || recipe.defaultSize || Object.keys(recipe.sizes)[0];
  ...
  if (availableMethods.length === 1) {
    ...
    methodCard.querySelectorAll('.method-option-card').forEach((card) => {
      card.addEventListener('click', () => {
        methodCard
          .querySelectorAll('.method-option-card')
...
btnNext2.addEventListener('click', () => {
  ...
  methodDisplayLabel =
    styleKey === 'chicago-tavern'
      ? 'Chicago Tavern \u2014 Cured Dough'
      : styleKey === 'school-night'
        ? 'No Rise'
  ...
  try {
    const _dough = calculateDough(recipe, numPizzas, sizeKey);
    PieLabStorage.set('pielab-last-calc', {
      styleKey,
      styleName: recipe.name,
      sizeKey,
      numPizzas,
...
function renderScheduleTimeline(steps) {
  ...
  if (bakeWithin4h && saved) {
    ...
    const ovenLabel = saved.ovenType ? OVEN_TYPES[saved.ovenType] : 'oven';
    ...
    let stored = {};
    try {
      stored = PieLabStorage.getJSON('pielab-bakeday-checklist') || {};
    } catch {
      /* ignore */
    }
  ...
  timelineEl.querySelectorAll('[data-bakeday-idx]').forEach((cb) => {
    cb.addEventListener('change', () => {
      ...
      PieLabStorage.set('pielab-bakeday-checklist', stored);
    });
  });
...
function renderRecipeSummary(styleKey, numPizzas, doughBallWeight, selectedSizeKey) {
  ...
  const sizeKey =
    selectedSizeKey || recipe.defaultSize || (sizeKeys.includes('12') ? '12' : sizeKeys[0]);
  const dough = calculateDough(recipe, numPizzas, sizeKey);

  const rows = dough
    .map((d) => `<tr><td>${d.ingredient}</td><td>${d.amount}g</td><td>${d.pct}%</td></tr>`)
    .join('');
  ...
  if (toggleEl && !toggleEl._bound) {
    ...
    toggleEl.addEventListener('click', () => {
      ...
      toggleEl.querySelector('.sched-recipe-arrow').style.transform = isOpen
        ? 'rotate(180deg)'
        : 'rotate(0deg)';
    });
    // Start expanded
...
function updateSchedCountdown(el) {
  ...
  if (diff <= 0) {
    ...
    if (!el.dataset.advanced) {
      ...
      const stepLabel =
        computedSchedule && stepIdx != null ? computedSchedule[stepIdx].label : 'This step';
      startStepAlarm();
...
function saveActiveSchedule(data) {
  PieLabStorage.remove('pielab-bakeday-checklist'); // reset checklist for new schedule
  try {
    PieLabStorage.set(STORAGE_KEY, data);
  } catch (e) {
...
function loadActiveSchedule() {
  try {
    const data = PieLabStorage.getJSON(STORAGE_KEY);
    if (!data) return null;
...
function clearActiveSchedule() {
  PieLabStorage.remove(STORAGE_KEY);
}
...
function restoreSchedule(data) {
  ...
  try {
    ...
    if (recipe && !PieLabStorage.get('pielab-last-calc')) {
      PieLabStorage.set('pielab-last-calc', {
...
(function checkPrefill() {
  ...
  const prefill = PieLabStorage.getJSON('pielab-plan-prefill');
  if (!prefill) return;

  PieLabStorage.remove('pielab-plan-prefill');
  ...
  if (eatTime <= now) {
    ...
    showPrefillBanner('Your planned eat time has passed.', 'Set a new time');
  ...
  const doughBallWeight = prefill.calcResult ? prefill.calcResult.doughBallWeight : null;
  ...
  if (!result.isValid) {
    ...
    showPrefillBanner(result.validationMsg || 'Could not build schedule.', 'Start over');
    if (prefill.styleKey && styleSelect.querySelector(`option[value="${prefill.styleKey}"]`)) {
      styleSelect.value = prefill.styleKey;
    }
    if (prefill.quantity) countInput.value = prefill.quantity;
  ...
  methodDisplayLabel =
    prefill.styleKey === 'chicago-tavern'
      ? 'Chicago Tavern \u2014 Cured Dough'
      : prefill.styleKey === 'school-night'
        ? 'No Rise'
        : method.label;
  ...
  showPrefillBanner('Schedule built from your Make calculation.', 'Start over');
